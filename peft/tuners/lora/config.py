...
class LoraConfig(PeftConfig):
    ...
    lora_variant: Literal["gsr", "vanilla", "alora"] = field(
        default="gsr",
        metadata={
            "help": (
                "Local project LoRA forward variant. `gsr` keeps the task-routed GSR-LoRA behavior, "
                "`vanilla` uses standard LoRA, and `alora` uses standard LoRA with fixed per-rank gates "
                "for ALoRA allocation baselines."
            )
        },
    )

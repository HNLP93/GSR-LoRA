...
def _torch_load_training_state(*args, **kwargs):
    kwargs = dict(kwargs)
...
class Trainer:
    ...
    def _load_rng_state(self, checkpoint):
        ...
        checkpoint_rng_state = _torch_load_training_state(rng_file)
    ...
    def _load_optimizer_and_scheduler(self, checkpoint):
            ...
            if not isinstance(self.lr_scheduler, DeepSpeedSchedulerWrapper):
                with warnings.catch_warnings(record=True) as caught_warnings:
                    self.lr_scheduler.load_state_dict(
                        _torch_load_training_state(os.path.join(checkpoint, SCHEDULER_NAME))
                    )
                reissue_pt_warnings(caught_warnings)
            return
        ...
        if checkpoint_file_exists and os.path.isfile(os.path.join(checkpoint, SCHEDULER_NAME)):
            ...
            if is_torch_xla_available():
                ...
                optimizer_state = _torch_load_training_state(
                    os.path.join(checkpoint, OPTIMIZER_NAME),
                    map_location="cpu",
                )
                with warnings.catch_warnings(record=True) as caught_warnings:
                    lr_scheduler_state = _torch_load_training_state(
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        """
        # layer = 6
        # layer_gate = model.module.base_model.model.encoder.block[layer].layer[0].SelfAttention.q.lora_gate.default
        # #print(layer_gate[0].weight)
        # layer_task_embedding = model.module.base_model.model.encoder.block[layer].layer[0].SelfAttention.q.lora_task_embedding.default
        # print(layer_task_embedding.weight)
